...
function remoteTagExists(tag) {
  try {
    run(`git ls-remote --tags ${remote} refs/tags/${tag}`);
    return true;
  } catch {
    return false;
  }
...
function planRelease() {
  ...
  const version = bumpLevel ? bumpVersion(oldVersion, bumpLevel) : oldVersion;
  const tag = `v${version}`;
  const commit = run('git rev-parse --short HEAD');

  if (tagExists(tag)) {
    fail(`本地已存在 tag ${tag}，请调整版本或删除旧 tag`);
  }
  if (remoteTagExists(tag)) {
    fail(`远程 ${remote} 已存在 tag ${tag}`);
  }
...
async function main() {
  ...
  if (bumpLevel) {
    ...
    run(`git commit -m "chore: bump version ${oldVersion} → ${version}"`);
  }
