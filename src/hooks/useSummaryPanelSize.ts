...
import { SUMMARY_PANEL_WIDTH_STORAGE_KEY } from '../utils/settingsStorage';
...
function readStoredSize(): number {
  try {
    const saved = localStorage.getItem(SUMMARY_PANEL_WIDTH_STORAGE_KEY);
...
export function useSummaryPanelSize() {
  ...
  const updateSize = useCallback((nextSize: number) => {
    ...
    setSize(clamped);
    try {
      localStorage.setItem(SUMMARY_PANEL_WIDTH_STORAGE_KEY, String(clamped));
    } catch {
